...
func applyJobFilter(buf []byte, job *jobs.Job) ([]byte, error) {
	switch job.Type {
	case jobs.JobType_JOB_TYPE_GRAYSCALE:
		return ApplyGrayscale(buf)
	...
	case jobs.JobType_JOB_TYPE_BRIGHTNESS:
		...
		return ApplyBrightness(buf, factor)
	case jobs.JobType_JOB_TYPE_RESIZE:
		...
		if originalHeight > 0 && originalWidth > 0 && targetHeight > 0 && targetWidth > 0 {
			...
			return ApplyResize(buf, newFragWidth, newFragHeight)
		}

		return buf, nil
	case jobs.JobType_JOB_TYPE_UNSPECIFIED,
		jobs.JobType_JOB_TYPE_RECONSTRUCT,
		jobs.JobType_JOB_TYPE_SLICE,
		jobs.JobType_JOB_TYPE_VIDEO_EXTRACT,
		jobs.JobType_JOB_TYPE_VIDEO_REASSEMBLE:
		return buf, nil
	}
...
func (p *Processor) handleProcess(ctx context.Context, job *jobs.Job) error {
	...
	processed, err = applyJobFilter(buf, job)
	if err != nil {
	...
	_, err = p.minio.UploadObject(ctx, model.BucketName, resultPath, bytes.NewReader(processed), int64(len(processed)), "image/png")
	if err != nil {
...
func (p *Processor) triggerReconstruction(ctx context.Context, job *jobs.Job) error {
	...
	if err := p.redis.PushTask(ctx, reconstructJob); err != nil {
...
func (p *Processor) handleVideoExtract(ctx context.Context, job *jobs.Job) error {
	...
	tmpFrames := "/tmp/frames_" + job.ParentId
	defer func() { _ = os.RemoveAll(tmpFrames) }()

	frames, width, height, _, err := ExtractFrames(tmpVideo, tmpFrames)
	if err != nil {
	...
	for _, frame := range frames {
		...
		_, err = p.minio.UploadObject(ctx, model.BucketName, framePath, bytes.NewReader(frameData), int64(len(frameData)), "image/png")
		if err != nil {
		...
		subJobParams := make(map[string]string)
		maps.Copy(subJobParams, job.Parameters)
		subJobParams["index"] = strconv.Itoa(frame.Index)
		subJobParams["total_subtasks"] = strconv.Itoa(len(frames))
...
func (p *Processor) handleVideoReassemble(ctx context.Context, job *jobs.Job) error {
	...
	tmpFrames := "/tmp/reassemble_" + job.ParentId
	defer func() { _ = os.RemoveAll(tmpFrames) }()

	if err := os.MkdirAll(tmpFrames, 0o755); err != nil {
	...
	for i := range totalFrames {
		...
		if err := os.WriteFile(tmpPath, frameData, 0o644); err != nil {
