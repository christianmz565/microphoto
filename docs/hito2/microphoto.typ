...
El problema se agrega en entornos de cómputo heterogéneos donde los nodos de trabajo poseen capacidades variables. Un sistema de procesamiento de imágenes distribuido de grado productivo debe abordar varios desafíos: balanceo de carga entre workers heterogéneos, tolerancia a fallos para tareas de larga ejecución, retroalimentación de progreso en tiempo real a los usuarios y reconstrucción eficiente de fragmentos procesados en paralelo.
...
#figure(
  image("img/architecture.png", width: 70%),
...
El flujo completo de datos procede a través de seis fases, como se muestra en la @fig:flowchart. Primero, el cliente sube una imagen mediante un POST multipart, el Coordinator la almacena en el objeto de devolución y retorna un ID de tarea. Segundo, el cliente se suscribe al SSE para obtener progreso en tiempo real. Tercero, un Worker extrae el trabajo SLICE, fragmenta la imagen y encola N subtareas de procesamiento. Cuarto, múltiples Workers procesan fragmentos en paralelo, cada uno decrementando un contador atómico de Redis al completar. Quinto, el último Worker, cuando el contador llega a cero, adquiere un bloqueo de reconstrucción y activa el reensamblaje de la imagen. Sexto, el cliente obtiene el resultado final procesado.
...
#block(breakable: false)[
  #figure(
    table(
      ...
      [Frontend], [Coordinator], [SSE], [Progreso en tiempo real],
...
Los manejadores de procesamiento aplican la pipeline de efectos, escala de grises, desenfoque, brillo y redimensionamiento, utilizando libvips, suben los resultados a MinIO y decrementan atómicamente el contador de Redis. El Worker que observa que el contador $<= 0$ se convierte en el coordinador de reconstrucción, utilizando SetNx en una clave de deduplicación para prevenir activaciones duplicadas.
...
La @tab:job-types escribe los once tipos de trabajos que definen el ciclo de vida del procesamiento.
...
#block(breakable: false)[
  #figure(
    table(
      columns: (auto, 1fr),
      align: (left, left),
      table.header([Operación], [Descripción]),
      [SLICE], [Descompone la imagen en fragmentos equilibrados por conteo de píxeles],
      [GRAYSCALE], [Convierte la imagen a escala de grises],
      [BLUR], [Aplica desenfoque gaussiano con radio configurable],
      [BRIGHTNESS], [Ajusta el brillo de la imagen],
      [CONTRAST], [Ajusta el contraste de la imagen],
      [SEPIA], [Aplica efecto sépia a la imagen],
      [VIGNETTE], [Aplica efecto de viñeta],
      [RESIZE], [Redimensiona la imagen proporcionalmente],
      [RECONSTRUCT], [Reensambla los fragmentos procesados en la imagen final],
      [VIDEO_EXTRACT], [Extrae segmentos y frames del video de entrada],
      [VIDEO_REASSEMBLE], [Concatena segmentos procesados en el video final],
    ),
    caption: [Tipos de trabajos y sus descripciones en el ciclo de vida del procesamiento.],
  ) <tab:job-types>
]

== Tolerancia a Fallos

La operación BLMOVE mueve atómicamente elementos de la cola a una lista en progreso, asegurando entrega a lo sumo-una-vez. Si un Worker falla a mitad del procesamiento, el elemento permanece en la lista en progreso para recuperación por el Reaper.

La operación DECR de Redis en el contador de subtareas sirve como una barrera distribuida. El Worker que observa que el contador $<= 0$ es responsable de activar la siguiente etapa del pipeline. Combinada con la deduplicación SetNx, esto asegura reconstrucción exactamente-una-vez.

El Reaper detecta trabajos estancados comparando las marcas de tiempo contra un tiempo máximo configurable, reprogramándolos con un presupuesto restante de intentos. Tras agotar los reintentos, el trabajo se marca como fallido y un evento JOB_FAILED se propaga al cliente.
...
#figure(
  table(
    ...
    table.header([Nodo], [Rol], [SO]),
    [ynoacamino-instance], [control-plane], [Ubuntu 22.04 (Oracle Cloud)],
    [cricro-vm], [worker], [NixOS 26.05],
    [gustadev-server-dell], [worker], [Ubuntu 24.04],
    [sd-node-do], [worker], [Debian 12 (DigitalOcean)],
    [cricro-laptop], [worker], [NixOS 26.11],
    [cricro-pc], [worker], [NixOS 26.11],
  ),
...
Las métricas de OpenTelemetry se exportan desde todos los servicios Go y son scrapingeadas por Prometheus. El dashboard de Grafana proporciona cuatro paneles de monitoreo, como se muestra en la @fig:grafana, se presenta utilización de CPU por nodo, carga promedio en el clúster, uso de swap y consumo de memoria. Se rastrean tres métricas predefinidas: tasks_processed_total, contador por ID de worker y tipo de tarea, task_duration_seconds, histograma por ID de worker, y task_timeouts_total, contador por ID de worker.
...
La @fig:processing muestra el dashboard de procesamiento de Microphoto durante una sesión de procesamiento distribuido en vivo. El indicador de progreso muestra 95\% de completado en la fase de reensamblaje, con tres etapas de verificación completadas: extracción de frames, procesamiento de frames y reensamblaje de video. Debajo, aproximadamente 30 nodos workers activos muestran marcas de tiempo de procesamiento por fragmento, demostrando la capacidad del sistema para distribuir trabajo a través del clúster.
...
El editor web, la @fig:editor, proporciona acceso inmediato a las capacidades de procesamiento. Los usuarios pueden ajustar la intensidad de escala de grises de 0 a 100%, el radio de desenfoque de 0 a 20px y el factor de brillo de 0.5 a 2.0x mediante controles deslizantes intuitivos. El botón "Procesar en Cluster" activa el pipeline distribuido, mientras que el sistema de vista previa proporciona retroalimentación instantánea usando manipulación de imágenes dentro del proceso.
...
La @fig:result muestra una salida de video procesada del pipeline distribuido. El sistema mantiene fidelidad visual completa a través del ciclo de fragmentar-procesar-reconstruir, sin artefactos visibles en los límites de los fragmentos gracias al mecanismo de padding de desenfoque.
...
Las contribuciones técnicas clave incluyen una estrategia de fragmentación que equilibra la carga de procesamiento al apuntar a un conteo fijo de píxeles por fragmento, un mecanismo de sincronización basado en Redis que combina contadores DECR con bloqueos SetNx para reconstrucción exactamente-una-vez, un sistema de progreso de doble entrega que combina listas de eventos duraderas con pub/sub en tiempo real, y un despliegue productivo en un clúster k3s heterogéneo de 7 nodos con autoescalado HPA.

El trabajo futuro se enfocará en tres direcciones: procesamiento de fragmentos acelerado por GPU utilizando Workers habilitados con CUDA para filtros computacionalmente intensivos, tamaño de fragmentos adaptativo basado en métricas de rendimiento de workers en tiempo real para optimizar el balanceo de carga, y despliegue multi-región con Workers geo-distribuidos para latencia reducida en escenarios de computación en el borde.
